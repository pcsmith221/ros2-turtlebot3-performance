...
def ParseGazeboLog(gazeboLogLocation, correctNumberOfDigits):
    ...
    #some Gazebo timestamps have less digits than the rest for some reason, so we drop any rows with an incorrect number of digits or the time index will be messed up
...
def CalculateDistanceFromGoal(tfLogLocation, turtlebotPosesDf, goalCoordinate):
    """Returns Euclidean distance from goal"""
